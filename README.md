...
- The current logic blocks access if the users screenName is not 'test'. This should be replaced with the required custom logic.
- Ensure the module and any changes are fully tested in a non-production environment for all required use cases and scenarios, **before** considering deploying to a production environment.
- All logging is currently INFO for testing purposes. Consider changing before deploying in production, otherwise it may generate significant amounts of logging.
...
- The custom component will block requests where the requestURI matches **/api/jsonws** or **/api/jsonws/** (both case insensitive) or has a request parameter named **contextPath**.
  ...
  - The solution intentionally does not restrict access to **/api/jsonws/invoke** as it is used by Liferay.Service JavaScript API to invoke endpoints.
